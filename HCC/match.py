...
cur.execute("DROP TABLE IF EXISTS genematch") #if this table doesn't exist, create it. SQLITE3
cur.execute("CREATE TABLE genematch (sample text, geneid text, ntcount real, tncount real)")# determine the vals in each col of table
cur.execute("DROP TABLE IF EXISTS isoformmatch"); # a second table
cur.execute("CREATE TABLE isoformmatch (sample text, geneid text, isoformid text, ntcount real, tncount real)")
